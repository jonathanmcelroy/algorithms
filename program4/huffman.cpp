...
Description:
    This program will either encode or decode a file using Huffman Encoding according to command line arguments.
Due Date: 03/23/2014
Date Created: 03/16/2014
Date Last Modified: 03/23/2014
*******************************************************************************/
/******************************************************************************
Data Abstraction:
    -huff:
...
const char USAGE[] = "Use this string";
...
ostream& Huffleaf::print(ostream& out) const {
    return out << this->ch;
}
...
ostream& Huffinner::print(ostream& out) const {
    out << "{";
    if (this->left) {
        this->left->print(out);
        out << " /\\";
...
Hufftree::Hufftree(map<char, int> m) {
    // the heap/priority queue to build the tree
...
void Outputer::write(string bytes) {
    // should receive a string containing '0's and '1's. Will encode the string
    // in a series of bytes and write them to the stream when ready.
...
int main(int argc, char* argv[]) {
    ...
    //decrypt("destination", "destination2");
